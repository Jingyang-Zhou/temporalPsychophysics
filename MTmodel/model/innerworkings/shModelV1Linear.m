...
nScales = pars.nScales; % 1 for linear v1 response
if nargin > 2
    resdirs = varargin{3};
end

% make sure the inputs are okay.
mSz = [size(M, 1), size(M, 2), size(M, 3)]; % stimulus dimensions
...
order = 3;
fsz = size(v1SpatialFilters, 1); % spatial filter size, spatial filter: length x 4 (size)
...
for scale = 1:nScales
    ...
    for torder = 0:order
        ...
        for xorder = 0:(order-torder)
            ...
            % inter polate the three directional derivatives, see appendix
            % 1
            ...
            % interpolate, 10 triplelets of derivatives
            n = n + 1;
        end
    end
end
...
% interpolation.
pop = S*shSwts(v1PopulationDirections)';                 % interpolate to get pop     % compute directional responses
