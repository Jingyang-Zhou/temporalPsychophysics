...
help mkDots
dir = 1; speed = 1; 
s = mkDots([37, 37, 50], dir, speed); 
%s = mkDots([37 37 256], dir, speed, 0.1, 1, -1, 'exact'); 

% load Burr & Santoro stimulus
%a = load(fullfile(tBevRootPath, 'output', 'CtrCohStim')); 
%s = a.cohStim{2,7}; % size of s = 324x324x78 at 100% coherence
...
rspop = reshape(pop, [ind(2, 2), ind(2, 3), ind(2, 4), size(pop, 2)]);
for k = 1 : ind(2, 4)
   imagesc(rspop(:, :, k, 3))
   pause(0.5)
end
